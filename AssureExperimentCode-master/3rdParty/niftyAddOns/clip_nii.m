...
%  - Jimmy Shen (jimmy@rotman-baycrest.on.ca)
%
function nii = clip_niiDror(nii, opt)
...
%my
dim1 = dims(1) - cut_from_R - cut_from_L;
dim2 = dims(2)-cut_from_A - cut_from_P;
dim3 = dims(3)-cut_from_I - cut_from_S;
dimsNew = [dim1 dim2 dim3];
originNew = round((dimsNew+1)/2);

nii = make_nii(nii.img( (cut_from_L+1) : (dims(1)-cut_from_R), ...
    (cut_from_P+1) : (dims(2)-cut_from_A), ...
    (cut_from_I+1) : (dims(3)-cut_from_S), ...
    :,:,:,:,:), nii.hdr.dime.pixdim(2:4), ...
    [originNew(1) originNew(2) originNew(3)], ...
    nii.hdr.dime.datatype, nii.hdr.hist.descrip);
