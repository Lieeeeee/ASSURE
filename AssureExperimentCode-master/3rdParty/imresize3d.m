function out=imresize3d(im,tsize)

if length(size(im))< 3 && length(size(im)) > 4
    error 'invalid usage';
end

if length(size(im))==3
    out = imresize3dHelper(im,[],tsize);
else
    out = zeros([tsize,size(im,4)]);
    for ii=1:size(im,4)
        out(:,:,:,ii) = imresize3dHelper( im(:,:,:,ii),[],tsize);
    end
end
...
function A=imresize3dHelper(V,scale,tsize,ntype,npad)
